...
public class RecommendUser
{
     ...
     * @return
     */
    private Map<Double, Integer> computeNearestNeighbor(int uid, UserSet set) {

        Map<Double, Integer> distances = new TreeMap<>();

        UserSet.User u1 = set.getUser(uid);
        for (int i = 0; i < set.users.size(); i++) {
            UserSet.User u2 = set.getUserByposition(i);

            if (!(u2.uid == uid)) {
                double distance = pearson_dis(u2.list, u1.list);
                distances.put(distance, u2.uid);
            }
...
//    public List<UserSet.Set> recommend(int uid, UserSet set) {
//        //找到最近邻
//        Map<Double, Integer> distances = computeNearestNeighbor(uid, set);
...
//    }
    public int recommend(int uid, UserSet set) {
        //找到最近邻
        Map<Double, Integer> distances = computeNearestNeighbor(uid, set);
        int nearest = distances.values().iterator().next();
        return nearest;
